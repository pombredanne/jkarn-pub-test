...
%default INPUT_PATH 's3n://mortar-example-data/airline-data/634173101_T_ONTIME.csv'
%default OUTPUT_PATH 's3n://hawk-example-output-data/$MORTAR_EMAIL_S3_ESCAPED/mortar-examples/airline-data'

REGISTER '../udfs/python/airline_travel.py' USING streaming_python AS airline_travel;

-- Load airline data
raw_data = LOAD 's3://mortar-example-data/airline-data/634173101_T_ONTIME.csv' 
           USING org.apache.pig.piggybank.storage.CSVExcelStorage('SKIP_HEADER')
           AS ( 
               year:int, month: int, 
               unique_carrier:chararray, origin_airport_id:chararray, dest_airport_id:chararray,
...
apply_penalty = FOREACH raw_data 
                GENERATE unique_carrier, origin_airport_id, dest_airport_id, airline_travel.null_to_zero(dep_delay_new) 
                AS dep_delay, airline_travel.create_penalty(arr_delay_new) AS arr_delay;
...
-- This penalizes an airline for having a larger-than-average arrival delay, and attempts to normalize for flights 
-- that leave from delayed versus less delayed airports
normalized_data_final = FOREACH join_both_delay GENERATE 
                            unique_carrier, 
                            ((arr_delay - avg_arrival_delay) - avg_departure_delay) AS normalized_arrival_delay;

-- Get the normalized badness for each airline
normalized_data_by_airline = GROUP normalized_data_final BY (unique_carrier);
avg_normalized_delay_airline = FOREACH normalized_data_by_airline GENERATE 
                                   group, 
                                   AVG(normalized_data_final.normalized_arrival_delay) AS normalized_arrival_delay;
